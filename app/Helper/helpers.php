...
function calculate_profile_completion($user_id)
{
    ...
    $percent = 15; // Registered user base
    ...
    $experienceCount = UserExperience::where('user_id', $user_id)->count();
    $educationCount = UserEducation::where('user_id', $user_id)->count();

    // 🔁 Replace UserWork with user_work_subcategories table
    $subcategoriesCount = DB::table('user_work_subcategories')
        ->where('user_id', $user_id)
        ->count();

    // Optional: if user has skills relation
    $skillsCount = $user->skills()->count() ?? 0;

    if ($intro && $experienceCount > 0 && $educationCount > 0 && $subcategoriesCount > 0 && $skillsCount > 0 && $user->hourly_rate) {
        $percent += 25;
    }
    ...
    if (
        ...
        $bank->account_title &&
        $bank->bank_name &&
        ($bank->iban_number || $bank->swis_code || $bank->account_number)
    ) {
        $percent += 25;
    }
...
function site_currency_symbol($text = false)
{
    if (class_exists(\App\Helper\CurrencyHelper::class)) {
        return \App\Helper\CurrencyHelper::getSymbol($text);
    }
...
{
    if (class_exists(\App\Helper\CurrencyHelper::class)) {
        $amount = \App\Helper\CurrencyHelper::convert($amount);
    }
    ...
    $avg_rating = $count > 0 ? $total_rating/$count : 0;
        ...
        if($project_complete_order_count >= 1 && $avg_rating >=1 ){
            $string = '<div class="single-project-content-review">
                        <span class="single-project-content-review-icon"> <i class="fa-solid fa-star starss"></i> </span>
                        <span class="single-project-content-review-rating">'.round($avg_rating,1).'('.$rating_count.') </span>
                   </div>
                     <a href="#/" class="single-project-orderCompleted"> '.$project_complete_order_count.' '.__("Completed Projects").' </a>';
    ...
    if($header == null){
        if($complete_orders_count >= 1 && $avg_rating >= 1){
            $string = '<div class="jobFilter-proposal-author-contents-review-flex">
                        ...
                        <p class="jobFilter-proposal-author-contents-review-para" style="margin: 1px;">'.round($avg_rating,1).' <span>('.$rating_count.')</span> </p>
                    </div>
                    <a href="#/" class="single-project-orderCompleted"> '.$complete_orders_count.' '.__("Jobs Completed").'</a>';
        }elseif($avg_rating >= 1){
            $string = '<a href="#/" class="single-project-orderCompleted"> '.$complete_orders_count.' '.__("Jobs Completed").'</a>';
        }
        return $string;
    }else{
    ...
    if($complete_orders_count >= 1 && $rating_count >= 1){
        ...
        $string = ' <div class="single-project-content-review mt-2">
                           <span class="single-project-content-review-icon">
                               <i class="fa-solid fa-star starss"></i>
                           </span>
                           <span class="single-project-content-review-rating">' .round($avg_rating,1.).'('.$rating_count.')</span>
                    </div>
                     ';
...
{
    $all_gateways = ['wallet','paypal','manual_payment','mollie','paytm','stripe','razorpay','flutterwave','paystack','marcadopago','instamojo','cashfree','payfast','midtrans','squareup','cinetpay','paytabs','billplz','zitopay','sitesway','toyyibpay','authorize_dot_net','iyzipay','pagali','paypro'];
    return $all_gateways;
}
