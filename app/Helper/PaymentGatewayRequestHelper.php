...
class PaymentGatewayRequestHelper
{
    ...
    public static function payfast(){
        ...
        $payfast->setCurrency(get_static_option('site_global_currency') ?? 'USD');
        $payfast->setEnv(get_static_option('payfast_test_mode') == 'on' ? true : false);
        $payfast->setStoreId(get_static_option('payfast_store_id') ?? '');
        $payfast->setExchangeRate(self::pkrConversionValue()); // Convert USD to PKR for Pakistan PayFast
        return $payfast;
    }
    ...
    private static function globalCurrency()
    {
        if (class_exists(\App\Helper\CurrencyHelper::class)) {
            return \App\Helper\CurrencyHelper::getActiveCurrency();
        }
    ...
    {
        if (self::globalCurrency() === 'PKR') {
            if (class_exists(\App\Helper\CurrencyHelper::class)) {
                $pkr_rate = \App\Helper\CurrencyHelper::getExchangeRate();
                return 1.0 / $pkr_rate;
            }
            return 1.0 / 280.0;
        }
    ...
    private static function pkrConversionValue()
    {
        if (class_exists(\App\Helper\CurrencyHelper::class)) {
            return \App\Helper\CurrencyHelper::getExchangeRate();
        }
        return get_static_option('site_usd_to_pkr_exchange_rate') ?? 280.0;
    }
