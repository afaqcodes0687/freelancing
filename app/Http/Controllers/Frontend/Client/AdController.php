...
class AdController extends Controller
{
    ...
    public function pay(Request $request)
    {
        ...
        if ($gateway === 'paypro') {
            ...
            $click2PayUrl .= $separator . 'callback_url=' . urlencode(route('client.ad.paypro.return'));
        ...
        if ($gateway === 'payfast') {
            ...
            try {
                $callbackUrl = route('client.ad.payfast.return');
                $cancelUrl = route('client.ad.manage');
    ...
    public function paypro_return(Request $request)
    {
        ...
        if (!$adId) {
            return redirect()->route('client.ad.manage')->with(toastr_error(__('Ad not found or session expired.')));
        }

        $ad = Ad::where('id', $adId)->where('user_id', Auth::id())->first();
        if (!$ad) {
            return redirect()->route('client.ad.manage')->with(toastr_error(__('Ad not found.')));
        }

        if ($ad->is_paid) {
            return redirect()->route('client.ad.manage')->with(toastr_success(__('Payment already completed')));
        }
        ...
        try {
            ...
            \Log::info('PayPro Ad Status Check Result', ['response' => $statusData]);
            ...
            if ($apiPaid || strtolower($request->query('status')) === 'success') {
                ...
                return redirect()->route('client.ad.manage')->with(toastr_success(__('Payment successful')));

            } else {
                $orderStatus = $statusData[1]['OrderStatus'] ?? 'Processing';
                return redirect()->route('client.ad.manage')
                    ->with(toastr_warning(__('Payment status: ') . $orderStatus . '. If you paid, please wait a moment.'));
            }

        } catch (\Exception $e) {
            \Log::error('PayPro Ad Return Error', ['msg' => $e->getMessage()]);
            return redirect()->route('client.ad.manage')
    ...
    public function payfast_return(Request $request)
    {
        ...
        try {
            ...
            \Log::error('PayFast Ad Return Error (Client)', ['msg' => $e->getMessage()]);
            return redirect()->route('client.ad.manage')
                ->with(toastr_error(__('Unable to verify payment status. Please contact support.')));
        }
    }
