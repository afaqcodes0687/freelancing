...
class OrderController extends Controller
{
    ...
    {
        $client_id = Auth::guard('web')->user()->id;
        $order_details = Order::with(['freelancer:id,first_name,last_name,email,phone,country_id,state_id,city_id,image,username,user_verified_status', 'order_submit_history', 'job'])
            ->where('id', $id)
    ...
    public function order_milestone_approve($id, $type)
    {
        if ($type == 'milestone') {
            ...
            if ($next_milestone) {
                ...
                // ✅ Dispatch commission job on completion
                \App\Jobs\CommissionJob::dispatch((int) $milestone->order_id);
            ...
            // ✅ Dispatch commission job on completion
            \App\Jobs\CommissionJob::dispatch((int) $id);
