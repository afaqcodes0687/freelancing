...
class CategoryProjectController extends Controller
{
    ...
    {
        $is_pro = 0;
        $category = Category::select('id','category','slug','short_description','meta_title','meta_description','image')->where('slug',$slug)->first();
        if(!empty($category)){
            // Load subcategories with their child categories
            $category->load(['sub_categories' => function($q){
                $q->where('status', 1)->with(['child_categories' => function($q2){
                    $q2->where('status', 1)->select('id','sub_category_id','name','slug');
                }]);
            }]);

            // If category has subcategories, show hub page
            if($category->sub_categories->count() > 0){

                // No longer needed: flat_child_categories


                // Live stats
                $project_count    = \App\Models\Project::where('category_id', $category->id)
                    ->where('project_on_off','1')->where('status','1')->count();
                $freelancer_count = \App\Models\Project::where('category_id', $category->id)
                    ->where('project_on_off','1')->where('status','1')
                    ->distinct('user_id')->count('user_id');

                // Dynamic FAQs: category-specific first, then global (null category_id)
                $faqs = \Modules\Faq\Entities\QuestionAnswer::where('category_id', $category->id)
                    ->orWhereNull('category_id')
                    ->latest()
                    ->take(10)
                    ->get();

                return view('frontend.pages.category-projects.hub', compact(
                    'category', 'is_pro',
                    'project_count', 'freelancer_count', 'faqs'
                ));
            }

            // Fallback: No subcategories → show projects directly
