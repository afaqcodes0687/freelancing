...
class OrderIPNController extends Controller
{
    ...
    /**
     * Handle PayPro return URL after payment for Order
     */
    public function paypro_return_for_order(Request $request)
    {
        $orderId = $request->query('order') ?? $request->query('ordId');
        $sessionCtx = session('order_paypro');
        $id = $sessionCtx['order_id'] ?? null;

        if (!$id && $orderId) {
            // Try to parse ORD-ID-TIMESTAMP
        ...
        if (!$id) {
            return redirect()->route('homepage')->with(['msg' => __('Order not found or session expired.'), 'type' => 'error']);
        }

        $order = Order::find($id);
        if (!$order) {
            return redirect()->route('homepage')->with(['msg' => __('Order not found.'), 'type' => 'error']);
        }

        if ($order->payment_status === 'complete') {
            toastr_success('Order successfully completed');
            $new_order_id = getLastOrderId($order->id);
            return redirect()->route('order.user.success.page', $new_order_id);
        }
        ...
        try {
            ...
            $checkOrderNumber = $orderId ?? ($sessionCtx['order_number'] ?? null);
            if (!$checkOrderNumber) {
            ...
            \Log::info('PayPro Order Status Check Result', ['response' => $statusData]);
            ...
            if ($apiPaid || strtolower($request->query('status')) === 'success') {
                ...
                $this->update_database($order->id, $request->query('ordId') ?? $checkOrderNumber, $user_id, $freelancer_id, $project_or_job, $proposal_id);
                $this->send_order_mail($order->id, $user_id, $freelancer_id);

                session()->forget('order_paypro');
            ...
            \Log::error('PayPro Order Return Error', ['msg' => $e->getMessage()]);
            return redirect()->route('homepage')
    ...
    /**
     * Handle PayPro Webhook for Order
     */
    public function paypro_webhook_for_order(Request $request)
    {
        $payload = $request->all();
        \Log::info('PayPro Order Webhook Received', ['payload' => $payload]);

        $orderNumber = $request->input('OrderNumber');
        $status = $request->input('Status'); // '00' is success provided by PayPro docs

        if (!$orderNumber) {
            return response()->json(['status' => 'error', 'message' => 'OrderNumber missing'], 400);
        }

        // Parse order ID from OrderNumber (ORD-123-TIMESTAMP)
        $orderId = null;
        ...
        if (!$orderId) {
            return response()->json(['status' => 'error', 'message' => 'Invalid OrderNumber format'], 400);
        }
        ...
        if ($order->payment_status === 'complete') {
            return response()->json(['status' => 'success', 'message' => 'Order already complete']);
        }

        if ($status === '00') {
            // PayPro status '00' means success

            // Extract optional params for update_database
            $user_id = $order->user_id;
            $freelancer_id = $order->freelancer_id;
            $project_or_job = $order->is_project_job;

            // We may not have proposal_id in session for webhook, try to find from JobProposal if possible 
            // or assume it's already linked? 
            // update_database uses $proposal_id to set is_hired=1. 
            // If we don't have session, we need to look it up.
            // Order table doesn't seem to store proposal_id directly but we might find it?
            // Actually, update_database takes $proposal_id.
            // Let's assume for now we might miss proposal_id if not stored in Order.
            // Checking Order table schema from OrderService might be useful but we can try to guess or skip if null.
            // Wait, OrderService stores request->proposal_id_for_order in session, but not in Order table explicitly?
            // View_file of OrderService creates Order with 'identity' which IS proposal_id for job orders?
            // Line 67: 'identity' => $identity (where identity = proposal_id_for_order)

            $proposal_id = session()->get('proposal_id_for_order') ?? $order->identity;

            $this->update_database($order->id, $orderNumber, $user_id, $freelancer_id, $project_or_job, $proposal_id);

            return response()->json(['status' => 'success', 'message' => 'Order processed']);
        }

        return response()->json(['status' => 'ignored', 'message' => 'Status not successful']);
    }
    ...
    private function update_database($last_order_id, $transaction_id, $user_id, $freelancer_id, $project_or_job, $proposal_id)
    {
        ...
        if ($project_or_job == 'job') {
            ...
            // Check if proposal has milestones and copy to OrderMilestone
            $proposalMilestones = \App\Models\JobProposalMilestone::where('job_proposal_id', $proposal_id)->get();
            if ($proposalMilestones->count() > 0) {
                // Set order payment type to milestone if it has milestones
                Order::where('id', $last_order_id)->update(['payment_type' => 'milestone']);
                
                foreach ($proposalMilestones as $milestone) {
        ...
        // ✅ Create affiliate commission for this completed gateway order
        try {
            app(AffiliateCommissionService::class)->createForOrder($last_order_id);
        } catch (\Exception $e) { /* optionally log */
        }
