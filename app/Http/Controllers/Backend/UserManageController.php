...
use App\Models\User;
use App\Mail\VerificationDetailsMissingMail;
...
class UserManageController extends Controller
{
    ...
    public function add_user(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            Wallet::create([
                'user_id' => $user->id,
                'balance' => $request->user_type == 2 ? 10 : 0,
                'remaining_balance' => $request->user_type == 2 ? 10 : 0,
                'withdraw_amount' => 0,
                'status' => 1
            ]);
            ...
            try {
                $message = '
                    <div style="font-family: Arial, sans-serif; background-color: #f8f9fa; padding: 20px; border-radius: 10px;">
                        <h2 style="color: #343a40;">' . __('Welcome to ') . get_static_option("site_title") . '</h2>
                        <p style="font-size: 15px; color: #555;">' . __('Hello') . ' <strong>' . $user->first_name . ' ' . $user->last_name . '</strong>,</p>
                        <p style="font-size: 15px; color: #333;">' . __('Your account has been created by admin. Here are your login details:') . '</p>
                        <ul style="font-size: 14px; color: #444;">
                            <li><b>' . __('Email') . ':</b> ' . $user->email . '</li>
                            <li><b>' . __('Username') . ':</b> ' . $user->username . '</li>
                            <li><b>' . __('Password') . ':</b> ' . $request->password . '</li>
                        </ul>
                        <p style="font-size: 14px; color: #888;">' . __('You can login here:') . ' <a href="' . url('/login') . '">' . url('/login') . '</a></p>
                    </div>
                ';
                ...
                \Log::error('User welcome mail error: ' . $e->getMessage());
            }

            // Send notification to admin
            try {
                $admin_message = __('A new user has been created by Admin:') . ' <br>' .
                                __('Name') . ': ' . $user->first_name . ' ' . $user->last_name . '<br>' .
                                __('Email') . ': ' . $user->email . '<br>' .
                                __('Username') . ': ' . $user->username . '<br>' .
                                __('Type') . ': ' . ($user->user_type == 1 ? 'Client' : 'Freelancer');

                \Mail::to(get_static_option('site_global_email'))->send(new \App\Mail\BasicMail([
                    'subject' => __('New User Created by Admin'),
                    'message' => $admin_message
                ]));
            } catch (\Exception $e) {
                \Log::error('Admin notify mail error: ' . $e->getMessage());
            }
    ...
    public function edit_info(Request $request)
    {
        $request->validate([
            ...
            'edit_user_type'=>'required|in:1,2',
        ]);
        $user = User::findOrFail($request->edit_user_id);
        $hourly_rate = $request->edit_hourly_rate;
        if($request->edit_user_type == 1){
            $hourly_rate = 0;
        }else{
            $hourly_rate = $request->edit_hourly_rate ?? $user->hourly_rate;
        }

        if($user->user_type != $request->edit_user_type){
            $wallet = Wallet::where('user_id', $user->id)->first();
            if($wallet){
                if($user->user_type == 1 && $request->edit_user_type == 2){
                    // Client to Freelancer: +10
                    $wallet->increment('balance', 10);
                    $wallet->increment('remaining_balance', 10);
                } elseif($user->user_type == 2 && $request->edit_user_type == 1){
                    // Freelancer to Client: -10
                    $wallet->decrement('balance', 10);
                    $wallet->decrement('remaining_balance', 10);
                }
            }
        }
    ...
    {
        $all_requests = IdentityVerification::where(function ($query) {
                $query->whereNull('status')->orWhere('status', 2);
            })
            ->whereHas('user')->with([
                'user', 'user.user_country',
                'user.user_state',
                'user.user_city'
            ])->latest()->paginate(10);
    ...
    public function notify_missing_details(Request $request)
    {
        $request->validate([
            'user_id' => 'required|exists:users,id',
            'additional_message' => 'required|string',
            'verification_screenshot' => 'nullable|image|mimes:jpeg,png,jpg,gif|max:5120' // max 5MB
        ]);

        $user = User::select('id', 'first_name', 'last_name', 'email', 'username')->findOrFail($request->user_id);

        try {
            $mailData = [
                'subject' => __('Action Required: Identity Verification Details Missing'),
                'user_name' => $user->first_name . ' ' . $user->last_name,
                'additional_message' => $request->additional_message
            ];

            // Handle screenshot attachment
            if ($request->hasFile('verification_screenshot')) {
                // Get file content and convert to base64
                $file = $request->file('verification_screenshot');
                $imageContent = file_get_contents($file->getPathname());
                $base64Image = base64_encode($imageContent);
                $mimeType = $file->getMimeType();
                
                $mailData['screenshot_base64'] = $base64Image;
                $mailData['screenshot_mime'] = $mimeType;
                $mailData['screenshot_name'] = $file->getClientOriginalName();
            }

            Mail::to($user->email)->send(new VerificationDetailsMissingMail($mailData));

            return response()->json([
                'status' => 'success',
                'msg' => __('Professional notification sent successfully')
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
                'msg' => __('Failed to send notification: ') . $e->getMessage()
            ], 500);
        }
    }
