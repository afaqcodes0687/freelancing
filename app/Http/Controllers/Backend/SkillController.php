...
class SkillController extends Controller
{
   ...
   public function all_skill(Request $request)
   {
           ...
           $request->validate([
                'skill' => [
                    ...
                    Rule::unique('skills')->where(function ($query) use ($request) {
                        return $query->where('sub_category_id', $request->subcategory);
                    }),
                ],
               'category'=> 'required',
           ]);
       ...
       $all_skills = Skill::orderBy('id', 'desc')->paginate(10);
    ...
    public function edit_skill(Request $request)
    {
        $request->validate([
            ...
            Rule::unique('skills', 'skill')
                ->ignore($request->skill_id)
                ->where(function ($query) use ($request) {
                    return $query->where('sub_category_id', $request->edit_sub_category);
                }),
            ],
