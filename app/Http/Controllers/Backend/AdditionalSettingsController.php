...
class AdditionalSettingsController extends Controller
{
    ...
    public function signup_bonus_settings(Request $request)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                'signup_bonus_enable' => 'required|in:0,1',
                'signup_bonus_amount' => 'required|numeric|min:0',
            ]);

            update_static_option('signup_bonus_enable', $request->signup_bonus_enable);
            update_static_option('signup_bonus_amount', $request->signup_bonus_amount);

            toastr_success(__('Signup Bonus Settings Updated Successfully.'));
            return back();
        }
        return view('backend.pages.additional-settings.signup-bonus-settings');
    }

    public function subscription_tax_settings(Request $request)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                'subscription_tax_rate' => 'required|numeric|min:0',
            ]);

            update_static_option('subscription_tax_rate', $request->subscription_tax_rate);

            toastr_success(__('Subscription Tax Settings Updated Successfully.'));
            return back();
        }
        return view('backend.pages.additional-settings.subscription-tax-settings');
    }
