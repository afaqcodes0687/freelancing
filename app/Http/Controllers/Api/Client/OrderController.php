...
class OrderController extends Controller
{
    ...
    public function all_order()
    {
        ...
        if(get_static_option('project_enable_disable') != 'disable'){
            ...
            $orders = Order::with([
                'freelancer:id,first_name,last_name,image,user_verified_status,check_online_status,country_id,state_id,city_id,load_from',
    ...
    {
        $client_id = auth('sanctum')->user()->id;
        $order_type = Order::select('id','user_id','is_project_job')->where('id',$id)->first();

        if(!$order_type || $order_type->user_id != $client_id){
            return response()->json(['msg' => __('Order not found or access denied')]);
        }
        ...
        if($order_details?->freelancer?->image){
            $order_details->freelancer->freelancer_cloud_image = asset('assets/uploads/profile/'.$order_details?->freelancer?->image);
        }else{
    ...
    public function order_rating(Request $request, $id)
    {
        ...
        $order_check = Order::select('id', 'user_id', 'freelancer_id', 'status')
            ->where('id', $id)
            ->first();
        ...
        if ($order_check->user_id != $client_id) {
            return response()->json([
                'msg' => __('Access denied - Order does not belong to you'),
                'status' => 'error',
                'debug' => [
                    'order_id' => $id,
                    'order_owner_id' => $order_check->user_id,
                    'your_client_id' => $client_id,
                    'message' => 'Order belongs to different client'
                ]
            ], 403);
        }

        if (!in_array($order_check->status, [3, 4])) {
            return response()->json([
                'msg' => __('Order not eligible for rating'),
                'status' => 'error',
                'debug' => [
                    'order_id' => $id,
                    'order_status' => $order_check->status,
                    'eligible_statuses' => [3, 4],
                    'message' => 'Order must be completed (3) or cancelled (4)'
                ]
            ], 400);
        }

        $order = Order::with('freelancer:id,first_name')
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'freelancer_info' => [
                    ...
                    'name' => $order->freelancer->first_name,
                ]
            ]
        ]);
    }
