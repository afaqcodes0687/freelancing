...
class JobController extends Controller
{
    ...
    {
        $user_id = auth('sanctum')->user()->id;
        $all_jobs = JobPost::select(['id', 'title', 'description', 'type', 'budget', 'hourly_rate', 'estimated_hours', 'level', 'status', 'on_off', 'current_status', 'created_at'])
    ...
    public function job_details($id)
    {
        ...
        $job_details = JobPost::with([
            ...
            $query->withCount('complete_order_count_api')->withCount('freelancer_ratings')->withAvg('freelancer_ratings', 'rating')->with('milestones');
        },
        ...
        $completed_order = Order::where('identity', $id)
        ...
        $is_job_hired     = $hired_freelancer_count > 0;
    ...
    {
        $user_id = auth('sanctum')->user()->id;
        $job_proposals = JobProposal::where('job_id', $request->job_id)->where('client_id', $user_id)->with('milestones')->latest();
    ...
    public function rate_and_hours(Request $request)
    {
        ...
        if (!empty($job)) {
            ...
            return response()->json(['msg' => __('Hourly rate and hours updated successfully.')]);
        } else {
            return response()->json(['msg' => __('Job not found!')])->setStatusCode('404');
        }
    }
