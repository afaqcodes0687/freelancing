...
class ProjectController extends Controller
{
    ...
    public function project_list()
    {
        ...
        if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi'])) {
            $project_lists->transform(function ($project) {
                ...
                $project->full_image_url = $project->cloud_link;
                return $project;
            });
        } else {
            // For local storage, add full image URL
            $project_lists->transform(function ($project) {
                $project->full_image_url = asset('assets/uploads/project/' . $project->image);
                return $project;
            });
        }
    ...
    public function create_project(Request $request)
    {
        if($request->isMethod('post'))
        {
            ...
            // Return the created project with image URL
            $createdProject = Project::with(['project_category:id,category'])
                ->where('id', $project->id)
                ->first();
            
            if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi'])) {
                $createdProject->cloud_link = render_frontend_cloud_image_if_module_exists('project/'.$project->image, load_from: $project->load_from);
                $createdProject->full_image_url = $createdProject->cloud_link;
            } else {
                // For local storage, construct the full URL
                $createdProject->full_image_url = asset('assets/uploads/project/' . $project->image);
            }
