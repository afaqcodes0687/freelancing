...
<script>
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            // ── intl-tel-input for edit modal phone ─────────────────────────
            const editPhoneEl = document.querySelector('#edit_phone_iti');
            const editIti = window.intlTelInput(editPhoneEl, {
            ...
            editPhoneEl.addEventListener('countrychange', function () {
                if (typeof intlTelInputUtils !== 'undefined') {
                    var cc = editIti.getSelectedCountryData().iso2;
                    var ex = intlTelInputUtils.getExampleNumber(cc, false, intlTelInputUtils.numberType.MOBILE);
                    editPhoneEl.placeholder = ex || '';
                }
            });
            setTimeout(function () {
                if (typeof intlTelInputUtils !== 'undefined') {
            ...
            $(document).on('click', '.user_details', function () {
                ...
                $('#edit_user_details #edit_user_type').val($(this).data('type'));
                ...
                // set phone using intl-tel-input so country flag is shown
                if (phone) {
                    editIti.setNumber(phone);
                }
                ...
                if ($(this).data('type') == 1) {
                    $('.edit_hourly_rate_wrapper').hide();
                } else {
                    $('.edit_hourly_rate_wrapper').show();
                }
            });
            ...
            $(document).on('click', '.update_user_info', function () {
                ...
                // Sync ITI phone to hidden input before validation
                $('#edit_user_details #edit_phone').val(editIti.getNumber() || editPhoneEl.value);
                ...
                let user_type = $('#edit_user_details #edit_user_type').val();
                ...
                if (user_type == 2 && hourly_rate == '') {
                    toastr_warning_js("{{__('Please fill hourly rate field')}}")
                    return false;
                }
