                ...
                <div class="modal-body" id="edit_user_details">
                    <div class="edit_user_detailsInfo">
                        ...
                        <div class="single-input mb-1">
                            ...
                            {{-- visible intl-tel-input widget --}}
                            <input type="tel" id="edit_phone_iti" class="form-control"
                                placeholder="{{ __('Enter phone number') }}" style="width:100%;">
                            {{-- hidden field that carries the value to the backend --}}
                            <input type="hidden" name="edit_phone" id="edit_phone">
                        </div>
                        <span id="phone_availability"></span>
                        <div class="single-input mb-3">
                            <label class="label-title">{{ __('User Type') }}</label>
                            <select name="edit_user_type" id="edit_user_type" class="form-control">
                                <option value="1">{{ __('Client') }}</option>
                                <option value="2">{{ __('Freelancer') }}</option>
                            </select>
                        </div>
                        <div class="single-input mb-3 edit_hourly_rate_wrapper">
                            <x-form.text :title="__('Hourly Rate')" :type="'number'" :name="'edit_hourly_rate'"
                                :id="'edit_hourly_rate'" :placeholder="__('Enter hourly rate')" />
                        </div>
                        <div class="single-input mb-3">
