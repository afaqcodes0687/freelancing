        ...
        <tr>
            ...
            <td>
                @if($report->reporter == 'client')
                    {{ $report->client->first_name ?? '' }} {{ $report->client->last_name ?? '' }} ({{ __('Client') }})
                @elseif($report->reporter == 'freelancer')
                    {{ $report->freelancer->first_name ?? '' }} {{ $report->freelancer->last_name ?? '' }} ({{ __('Freelancer') }})
                @endif
            </td>
            <td>{{ $report->title }}</td>
            <td>
            ...
            <td>
                ...
                <ul class="dropdown-menu status_dropdown__list">
                    ...
                    @endif
                    <li class="status_dropdown__item">
                        <a href="{{ route('admin.user.report.delete', $report->id) }}" class="btn dropdown-item status_dropdown__list__link delete-report" data-id="{{ $report->id }}">
                            {{ __('Delete') }}
                        </a>
                    </li>
                </ul>
