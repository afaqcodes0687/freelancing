<script>
    (function ($) {
        "use strict";
        $(document).ready(function () {
            $('.country_select2').select2();

            // ── Child Category Tab Click (AJAX filter — no page reload) ──
            $(document).on('click', '.child-filter-tab', function () {
                var $this = $(this);

                // Update active tab
                $('.child-filter-tab').removeClass('active');
                $this.addClass('active');

                // Update hidden child_category_id
                var childId = $this.data('child-id');
                $('#child_category_id').val(childId ? childId : '');

                // Trigger project reload
                projects(1);
            });

            // star rating filter
            $(document).on('click', '.active-list .list', function() {
                let ratings = $(".active-list .list");
                ratings.each(function (){
                    $(this).removeClass('active');
                });
                $(this).addClass('active');
                projects();
            });

            // search by country / level / delivery_day
            $(document).on('change', '#country , #level , #delivery_day', function() {
                projects();
            });
            $(document).on('click', '#set_price_range', function() {
                projects();
            });

            // pagination
            $(document).on('click', '.pagination a', function(e){
                e.preventDefault();
                let page = $(this).attr('href').split('page=')[1];
                projects(page);
            });

            function projects(page = 1){
                let subcategory_id      = $('#subcategory_id').val();
                let child_category_id   = $('#child_category_id').val();
                let country             = $('#country').val();
                let level               = $('#level').val();
                let min_price           = $('#min_price').val();
                let max_price           = $('#max_price').val();
                let delivery_day        = $('#delivery_day').val();
                let get_pro_projects;
                let rating              = $('.filter-lists .list.active').attr('data-rating');

                if($('#get_pro_projects').prop('checked')){
                    $('#get_pro_projects').val('1');
                    get_pro_projects = '1';
                } else {
                    $('#get_pro_projects').val('0');
                    get_pro_projects = '0';
                }

                // Show loading state
                $('.search_subcategory_result').css('opacity', '0.5');

                $.ajax({
                    url: "{{ route('subcategory.project.pagination') . '?page=' }}" + page,
                    method: 'GET',
                    data: {
                        subcategory_id:    subcategory_id,
                        child_category_id: child_category_id,
                        country:           country,
                        level:             level,
                        min_price:         min_price,
                        max_price:         max_price,
                        delivery_day:      delivery_day,
                        rating:            rating,
                        get_pro_projects:  get_pro_projects
                    },
                    success: function(res){
                        $('.search_subcategory_result').css('opacity', '1');
                        if(res.status === 'nothing' || res.status === '{{ __("nothing") }}'){
                            $('.search_subcategory_result').html(
                                `<div class="congratulation-area section-bg-2 pat-100 pab-100">
                                    <div class="container">
                                        <div class="congratulation-wrapper">
                                            <div class="congratulation-contents center-text">
                                                <div class="congratulation-contents-icon bg-danger wow zoomIn animated" data-wow-delay=".5s">
                                                    <i class="fas fa-times"></i>
                                                </div>
                                                <h4 class="congratulation-contents-title"> {{ __('OPPS!') }} </h4>
                                                <p class="congratulation-contents-para">{{ __('Nothing') }} <strong>{{ __('Found') }}</strong></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>`);
                        } else {
                            $('.search_subcategory_result').html(res);
                        }
                    },
                    error: function(){
                        $('.search_subcategory_result').css('opacity', '1');
                    }
                });
            }

            // filter reset
            $(document).on('click', '#subcategory_project_filter_reset', function(e){
                e.preventDefault();
                $('#country').val('').trigger('change');
                $('#level').val('');
                $('#min_price').val('');
                $('#max_price').val('');
                $('#delivery_day').val('');
                $('.active-list .list').removeClass('active');
                projects();
            });

            // get pro projects toggle
            $(document).on('change', '#get_pro_projects', function(e){
                e.preventDefault();
                projects();
            });
        });
    }(jQuery));
</script>
