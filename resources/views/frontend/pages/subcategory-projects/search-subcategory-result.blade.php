                    ...
                    <div class="single-project-thumb">
                        <a href="{{ route('project.details', ['username' => $project->project_creator?->username, 'slug' => $project->slug]) }}">
                            @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                @php
                                    $imagePath = is_array($project->image) ? $project->image[0] : (is_string($project->image) ? explode(',', str_replace(['[',']','"'], '', $project->image))[0] : $project->image);
                                    $cloudImageUrl = $imagePath ? render_frontend_cloud_image_if_module_exists('project/'.$imagePath, load_from: $project->load_from) : null;
                                    $finalCloudImage = $cloudImageUrl ?: asset('assets/uploads/project/project-default-logo.jpeg');
                                @endphp
                                <img src="{{ $finalCloudImage }}" alt="{{ $project->title ?? '' }}">
                            @else
                                <!-- <img src="{{ asset('assets/uploads/project/'.$project->image) ?? '' }}" alt="{{ $project->title ?? '' }}"> -->
                                @php
                                    $imagePath = is_array($project->image) ? $project->image[0] : (is_string($project->image) ? explode(',', str_replace(['[',']','"'], '', $project->image))[0] : $project->image);
                                    $fullImagePath = public_path('assets/uploads/project/' . $imagePath);
                                    $finalImage = ($imagePath && file_exists($fullImagePath)) ? $imagePath : 'project-default-logo.jpeg';
                                @endphp
                                <img src="{{ asset('assets/uploads/project/'.$finalImage) }}" alt="{{ $project->title ?? '' }}">
