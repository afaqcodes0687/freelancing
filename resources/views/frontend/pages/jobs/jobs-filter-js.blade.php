    ...
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.read-more-btn', function() {
                let $btn = $(this);
                let $desc = $btn.closest('.job-description');
                let fullText = $desc.data('full');
                let shortText = fullText.substring(0, 300);

                if ($btn.text() === 'More') {
                    $desc.html(fullText + ' ');
                    $desc.append('<button class="btn btn-link p-0 read-more-btn">Less</button>');
                } else {
                    $desc.html(shortText + '<span class="dots">...</span> ');
                    $desc.append('<button class="btn btn-link p-0 read-more-btn">More</button>');
                }
            });
        });
    }(jQuery));
