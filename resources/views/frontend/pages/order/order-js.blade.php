        ...
        $(document).ready(function () {
            ...
            $(document).on('click','.basic_standard_premium',function(){
                ...
                if("{{ $transaction_charge > 0 }}"){
                    ...
                    let display_charge = transaction_charge;
                    if (transaction_type === 'fixed' && active_currency === 'PKR') {
                        display_charge = display_charge * exchange_rate;
                    }
                    
                    let transaction_amount = transaction_type == 'fixed' ? display_charge : (float_price*transaction_charge/100);
                ...
                let display_wallet_balance = wallet_balance;
                if (active_currency === 'PKR') {
                    display_wallet_balance = display_wallet_balance * exchange_rate;
                }
                if(float_price > display_wallet_balance){
                    $('.display_balance').html('<span class="text-danger">{{__('Wallet Balance Shortage:')}}'+ site_default_currency_symbol + (float_price-display_wallet_balance).toFixed(2) +'</span>');
