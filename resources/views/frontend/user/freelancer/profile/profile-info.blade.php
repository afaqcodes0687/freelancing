<div class="single-profile-settings mt-4" id="display_freelancer_profile_info">
    ...
    <div class="single-profile-settings-inner profile-border-top">
        <form id="inline_profile_form" method="post">
            @csrf
            <div class="single-profile-settings-form custom-form">
                ...
                <div class="single-input">
                    <label for="inline_level" class="label-title">{{ __('Your Experience Level') }}</label>
                    <select name="level" id="inline_level" class="form--control">
                        <option value="">{{ __('Select Experience Level') }}</option>
                        <option value="junior" {{ (Auth::guard('web')->user()->experience_level ?? '') == 'junior' ? 'selected' : '' }}>{{ __('Junior') }}</option>
                        <option value="mid-level" {{ (Auth::guard('web')->user()->experience_level ?? '') == 'mid-level' ? 'selected' : '' }}>{{ __('Mid Level') }}</option>
                        <option value="senior" {{ (Auth::guard('web')->user()->experience_level ?? '') == 'senior' ? 'selected' : '' }}>{{ __('Senior') }}</option>
                        <option value="expert" {{ (Auth::guard('web')->user()->experience_level ?? '') == 'expert' ? 'selected' : '' }}>{{ __('Expert') }}</option>
                    </select>
                </div>
