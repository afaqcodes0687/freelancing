    ...
    <div class="create-project-wrapper-item">
        ...
        <div class="create-project-intro-contents">
            <div class="create-project-intro-contents-form custom-form">
                ...
                <div class="single-input mt-3" id="child_category_wrapper" style="{{ $project_details->child_category_id ? '' : 'display: none;' }}">
                    <label class="label-title">{{ __('Select Child Category') }}</label>
                    <select name="child_category_id" id="child_category" class="form-control child_category_select2">
                        <option value="">{{ __('Select Child Category') }}</option>
                        @if($project_details->child_category_id && isset($get_child_categories_from_project_subcategory))
                            @foreach ($get_child_categories_from_project_subcategory as $childcategory)
                                <option value="{{ $childcategory->id }}" {{ $project_details->child_category_id == $childcategory->id ? 'selected' : '' }}>
                                    {{ $childcategory->name }}
                                </option>
                            @endforeach
                        @endif
                    </select>
                    <span id="child_category_info"></span>
                </div>
