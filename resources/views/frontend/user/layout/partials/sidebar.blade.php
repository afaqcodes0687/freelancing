<style>
   ...
   .incomplete {
    ...
    .dollar-game-link:hover {
        transform: translateX(4px);
    }

    .coin-icon {
        animation: spinCoin 3s linear infinite;
        filter: drop-shadow(0 0 6px #309400);
        margin-right: 8px;
    }
    ...
    .animated-gold-text {
        background: linear-gradient(90deg, #FFD700, #309400, #FFF3B0, #309400);
        background-size: 300% auto;
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        animation: shimmerGold 3.5s ease-in-out infinite;
        font-weight: 600;
        font-size: 15px;
    ...
    .dollar-game-link:hover .animated-gold-text {
        text-shadow: #309400 0 0 10px;
    }
</style>
    ...
    @php
        ...
        $steps = [];
        $clientStep1Complete = false;
        $clientStep2Complete = false;

        if ($user) {

            $user_introduction = \App\Models\UserIntroduction::where('user_id', $user->id)->first();
            $experiences = \App\Models\UserExperience::where('user_id', $user->id)->get();
            $educations = \App\Models\UserEducation::where('user_id', $user->id)->get();
            $user_work = \App\Models\UserWork::where('user_id', $user->id)->first();
            $bank_account = Modules\Wallet\Entities\BankAccount::where('user_id', $user->id)->first();
            $identity_verification = \App\Models\IdentityVerification::where('user_id', $user->id)->first();

            $step1Complete = $user->first_name && $user->last_name && $user->country_id && $user->experience_level && $user->image;
            $step2Complete = $user_introduction && $experiences->count() > 0 && $educations->count() > 0 && \App\Models\UserWorkSubcategory::where('user_id', $user->id)->exists() ;
            ...
            $steps = [
                ...
                [
                    'route' => 'freelancer.account.setup',
                    'text' => 'Account Setup',
                    'complete' => $step2Complete,
                ],
                [
                    'route' => 'freelancer.wallet.history',
                    'text' => 'Wallet',
                    'complete' => $step3Complete,
                ],
                [
                    'route' => 'freelancer.identity.verification',
                    'text' => 'Identity Verification',
                    'complete' => $step4Complete,
                ],
            ];

            // Client profile completion (2-step): Profile Settings + Identity Verification
            $clientStep1Complete = $user->first_name && $user->last_name && $user->country_id && $user->image;
            $clientStep2Complete = $identity_verification && $identity_verification->status === 1;
        } elseif (session()->has('logged_in_affiliate_id')) {
            // Affiliate user login hai
            $affiliate = \App\Models\AffiliateProgram::find(session('logged_in_affiliate_id'));

            // yahan affiliate ke liye apna data set karo
            // e.g., affiliate ka profile complete check
            $steps = [
                [
                    'route' => 'affiliate.dashboard',
                    'text' => 'Affiliate Dashboard',
                    'complete' => true, // affiliate ke liye steps ka apna logic lagao
                ],
            ];
        }
    @endphp
...
<div class="col-xl-3 col-lg-4">
    <div class="profile-settings-menu sticky_top_lg">
        ...
        <div class="profile-settings-menu-inner">
            ...
            @if (session()->has('logged_in_affiliate_id') && request()->is('affiliate/*'))
                @php
                    $affiliate = \App\Models\AffiliateProgram::find(session('logged_in_affiliate_id'));
                @endphp

               <ul class="profile-settings-menu-list">
                    <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.dashboard')) active @endif">
                        <a href="{{ route('affiliate.dashboard') }}" class="profile-settings-menu-item-link">
                    ...
                    <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.profile.settings')) active @endif">
                        <a href="{{ route('affiliate.profile.settings') }}" class="profile-settings-menu-item-link">
                    ...
                    <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.tools')) active @endif">
                        <a href="{{ route('affiliate.tools') }}" class="profile-settings-menu-item-link">
                            <svg width="24" height="24" viewBox="0 0 20 20" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.05 17.66a3 3 0 0 1 0-4.24l3.88-3.88a3 3 0 0 1 4.24 4.24l-1.41 1.41-1.41-1.41 1.41-1.41a1 1 0 1 0-1.41-1.41l-3.88 3.88a1 1 0 0 0 1.41 1.41l1.41-1.41 1.41 1.41-1.41 1.41a3 3 0 0 1-4.24 0Z" fill="#667085"/>
                            </svg>
                            {{ __('Affiliate Links') }}
                        </a>
                    </li>

                    <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.clicks')) active @endif">
                        <a href="{{ route('affiliate.clicks') }}" class="profile-settings-menu-item-link">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                            <rect x="4" y="12" width="3" height="8" fill="#667085"/>
                            <rect x="10" y="6" width="3" height="14" fill="#667085"/>
                            <rect x="16" y="10" width="3" height="10" fill="#667085"/>
                            </svg>
                            {{ __('Report') }}
                        </a>
                    </li>

                    <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.commissions')) active @endif">
                        <a href="{{ route('affiliate.commissions') }}" class="profile-settings-menu-item-link">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                            <ellipse cx="12" cy="7" rx="7" ry="3" fill="#667085"/>
                            <ellipse cx="12" cy="11" rx="7" ry="3" fill="#667085" opacity="0.7"/>
                            <ellipse cx="12" cy="15" rx="7" ry="3" fill="#667085" opacity="0.4"/>
                            </svg>
                            {{ __('Commissions') }}
                        </a>
                    </li>
                    
                     <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.payouts')) active @endif">
                        <a href="{{ route('affiliate.payouts') }}" class="profile-settings-menu-item-link">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                            <path d="M4 7h12a2 2 0 0 1 2 2v8a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3V9a2 2 0 0 1 2-2Zm14-2h-5l-1-2H8L7 5H4" fill="#667085"/>
                            <rect x="13" y="12" width="7" height="5" rx="1" fill="#667085"/>
                            </svg>
                            {{ __('Payouts') }}
                        </a>
                    </li>
                    ...
                    <li class="profile-settings-menu-item @if (request()->routeIs('affiliate.support')) active @endif">
                        <a href="{{ route('affiliate.support') }}" class="profile-settings-menu-item-link">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                            <path d="M6 10a6 6 0 1 1 12 0v2h-2v-2a4 4 0 1 0-8 0v2H6v-2Zm2 6a2 2 0 0 1-2-2v-1h12v1a2 2 0 0 1-2 2h-1v2h-2v-2H9v2H7v-2Z" fill="#667085"/>
                            </svg>
                            {{ __('Support') }}
                        </a>
                    </li>
                    <li class="profile-settings-menu-item">
                        <a href="{{ route('affiliate.logout') }}" class="profile-settings-menu-item-link">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                ...
                @php $user = Auth::guard('web')->user(); @endphp
                    ...
                    @if ($user->user_type == 2 && (request()->is('freelancer/*') || request()->routeIs('public.game.index')))
                    ...
                    <ul class="profile-settings-menu-list">
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.dashboard')) active @endif">
                         ...
                         <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.profile')) active @endif">
                            <a href="{{ route('freelancer.profile') }}" class="profile-settings-menu-item-link {{ !$step1Complete ? 'incomplete' : '' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M12 15.75C9.93 15.75 8.25 14.07 8.25 12C8.25 9.93 9.93 8.25 12 8.25C14.07 8.25 15.75 9.93 15.75 12C15.75 14.07 14.07 15.75 12 15.75ZM12 9.75C10.76 9.75 9.75 10.76 9.75 12C9.75 13.24 10.76 14.25 12 14.25C13.24 14.25 14.25 13.24 14.25 12C14.25 10.76 13.24 9.75 12 9.75Z"
                        ...
                        <li class="profile-settings-menu-item @if(!$step1Complete) disabled-link @endif">
                            <a href="{{ route('freelancer.account.setup') }}" class="profile-settings-menu-item-link {{ !$step2Complete ? 'incomplete' : '' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                          ...
                          <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.wallet.history')) active @endif @if(!$step2Complete) disabled-link @endif">
                                <a href="{{ route('freelancer.wallet.history') }}" class="profile-settings-menu-item-link {{ !$step3Complete ? 'incomplete' : '' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.identity.verification')) active @endif @if(!$step3Complete) disabled-link @endif">
                               <a href="{{ route('freelancer.identity.verification') }}" class="profile-settings-menu-item-link {{ !$step4Complete ? 'incomplete' : '' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                       ...
                       @if(get_static_option('project_enable_disable') != 'disable')
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.reports.all')) active @endif">
                            <a href="{{ route('freelancer.reports.all') }}" class="profile-settings-menu-item-link">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M13 11H8C7.45 11 7 10.55 7 10C7 9.45 7.45 9 8 9H13C13.55 9 14 9.45 14 10C14 10.55 13.55 11 13 11Z"
                                        fill="#667085" />
                                    <path
                                        d="M16 15H8C7.45 15 7 14.5 7 14C7 13.5 7.45 13 8 13H16C16.5 13 17 13.5 17 14C17 14.5 16.5 15 16 15Z"
                                        fill="#667085" />
                                    <path
                                        d="M12 19H8C7.45 19 7 18.5 7 18C7 17.5 7.45 17 8 17H12C12.5 17 13 17.5 13 18C13 18.5 12.5 19 12 19Z"
                                        fill="#667085" />
                                    <path
                                        d="M15.5 22.75H8.5C4.36 22.75 2.25 20.64 2.25 16.5V7.5C2.25 3.36 4.36 1.25 8.5 1.25H15.5C19.64 1.25 21.75 3.36 21.75 7.5V16.5C21.75 20.64 19.64 22.75 15.5 22.75ZM8.5 2.75C5.43 2.75 3.75 4.43 3.75 7.5V16.5C3.75 19.57 5.43 21.25 8.5 21.25H15.5C18.57 21.25 20.25 19.57 20.25 16.5V7.5C20.25 4.43 18.57 2.75 15.5 2.75H8.5Z"
                                        fill="#667085" />
                                </svg>
                                {{ __('Reports') }}
                            </a>
                        ...
                        <!-- <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.referral.list')) active @endif">
                            <a href="{{ route('freelancer.referral.list') }}" class="profile-settings-menu-item-link">
                               
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16 11C18.2091 11 20 9.20914 20 7C20 4.79086 18.2091 3 16 3C13.7909 3 12 4.79086 12 7C12 9.20914 13.7909 11 16 11Z" fill="#667085"/>
                                    <path d="M8 11C10.2091 11 12 9.20914 12 7C12 4.79086 10.2091 3 8 3C5.79086 3 4 4.79086 4 7C4 9.20914 5.79086 11 8 11Z" fill="#667085"/>
                                    <path d="M8 13C5.33 13 0 14.34 0 17V19C0 19.55 0.45 20 1 20H15C15.55 20 16 19.55 16 19V17C16 14.34 10.67 13 8 13Z" fill="#667085"/>
                                    <path d="M20 13H18V15H16V17H18V19H20V17H22V15H20V13Z" fill="#667085"/>
                                </svg>
                                {{ __('Refer a Friend') }}
                            </a>
                        </li> -->
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('freelancer.meeting.index')) active @endif">
                            <a href="{{ route('freelancer.meeting.index') }}" class="profile-settings-menu-item-link">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M19 10.75H17C14.58 10.75 13.25 9.42 13.25 7V5C13.25 2.58 14.58 1.25 17 1.25H19C21.42 1.25 22.75 2.58 22.75 5V7C22.75 9.42 21.42 10.75 19 10.75ZM17 2.75C15.42 2.75 14.75 3.42 14.75 5V7C14.75 8.58 15.42 9.25 17 9.25H19C20.58 9.25 21.25 8.58 21.25 7V5C21.25 3.42 20.58 2.75 19 2.75H17Z" fill="#667085"/>
                                    <path d="M7 22.75H5C2.58 22.75 1.25 21.42 1.25 19V17C1.25 14.58 2.58 13.25 5 13.25H7C9.42 13.25 10.75 14.58 10.75 17V19C10.75 21.42 9.42 22.75 7 22.75ZM5 14.75C3.42 14.75 2.75 15.42 2.75 17V19C2.75 20.58 3.42 21.25 5 21.25H7C8.58 21.25 9.25 20.58 9.25 19V17C9.25 15.42 8.58 14.75 7 14.75H5Z" fill="#667085"/>
                                    <path d="M12 12.75C9.93 12.75 8.25 11.07 8.25 9C8.25 6.93 9.93 5.25 12 5.25C14.07 5.25 15.75 6.93 15.75 9C15.75 11.07 14.07 12.75 12 12.75ZM12 6.75C10.76 6.75 9.75 7.76 9.75 9C9.75 10.24 10.76 11.25 12 11.25C13.24 11.25 14.25 10.24 14.25 9C14.25 7.76 13.24 6.75 12 6.75Z" fill="#667085"/>
                                </svg>
                                {{ __('Meetings') }}
                            </a>
                        </li>
                     ...
                     @elseif ($user->user_type == 1 && (request()->is('client/*') || request()->routeIs('public.game.index')))
                    ...
                    <ul class="profile-settings-menu-list">
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('client.profile')) active @endif">
                            <a href="{{ route('client.profile') }}" class="profile-settings-menu-item-link {{ !$clientStep1Complete ? 'incomplete' : '' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M12 15.75C9.93 15.75 8.25 14.07 8.25 12C8.25 9.93 9.93 8.25 12 8.25C14.07 8.25 15.75 9.93 15.75 12C15.75 14.07 14.07 15.75 12 15.75ZM12 9.75C10.76 9.75 9.75 10.76 9.75 12C9.75 13.24 10.76 14.25 12 14.25C13.24 14.25 14.25 13.24 14.25 12C14.25 10.76 13.24 9.75 12 9.75Z"
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('client.identity.verification')) active @endif @if(!$clientStep1Complete) disabled-link @endif">
                            <a href="{{ route('client.identity.verification') }}"
                                class="profile-settings-menu-item-link {{ !$clientStep2Complete ? 'incomplete' : '' }}">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M17 21.75H7C2.59 21.75 1.25 20.41 1.25 16V8C1.25 3.59 2.59 2.25 7 2.25H17C21.41 2.25 22.75 3.59 22.75 8V16C22.75 20.41 21.41 21.75 17 21.75ZM7 3.75C3.42 3.75 2.75 4.43 2.75 8V16C2.75 19.57 3.42 20.25 7 20.25H17C20.58 20.25 21.25 19.57 21.25 16V8C21.25 4.43 20.58 3.75 17 3.75H7Z"
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('client.live.chat')) active @endif">
                        ...
                        <li class="profile-settings-menu-item @if (request()->routeIs('client.meeting.index')) active @endif">
                            <a href="{{ route('client.meeting.index') }}" class="profile-settings-menu-item-link">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M19 10.75H17C14.58 10.75 13.25 9.42 13.25 7V5C13.25 2.58 14.58 1.25 17 1.25H19C21.42 1.25 22.75 2.58 22.75 5V7C22.75 9.42 21.42 10.75 19 10.75ZM17 2.75C15.42 2.75 14.75 3.42 14.75 5V7C14.75 8.58 15.42 9.25 17 9.25H19C20.58 9.25 21.25 8.58 21.25 7V5C21.25 3.42 20.58 2.75 19 2.75H17Z" fill="#667085"/>
                                    <path d="M7 22.75H5C2.58 22.75 1.25 21.42 1.25 19V17C1.25 14.58 2.58 13.25 5 13.25H7C9.42 13.25 10.75 14.58 10.75 17V19C10.75 21.42 9.42 22.75 7 22.75ZM5 14.75C3.42 14.75 2.75 15.42 2.75 17V19C2.75 20.58 3.42 21.25 5 21.25H7C8.58 21.25 9.25 20.58 9.25 19V17C9.25 15.42 8.58 14.75 7 14.75H5Z" fill="#667085"/>
                                    <path d="M12 12.75C9.93 12.75 8.25 11.07 8.25 9C8.25 6.93 9.93 5.25 12 5.25C14.07 5.25 15.75 6.93 15.75 9C15.75 11.07 14.07 12.75 12 12.75ZM12 6.75C10.76 6.75 9.75 7.76 9.75 9C9.75 10.24 10.76 11.25 12 11.25C13.24 11.25 14.25 10.24 14.25 9C14.25 7.76 13.24 6.75 12 6.75Z" fill="#667085"/>
