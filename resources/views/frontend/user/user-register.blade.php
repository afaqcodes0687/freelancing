...
@section('style')
    <style>
        ...
        .login-others-single-btn i.fa-google {
            color: #DB4437;
        }

        .login-others-single-btn i.fa-facebook {
            color: #1877F2;
        }

        .login-others-single-btn:hover i.fa-google {
            color: #c1351d;
        }

        .login-others-single-btn:hover i.fa-facebook {
            color: #0d5bd7;
        }
...
@section('content')
    @php
        ...
        $recaptcha_site_key = get_static_option('recaptcha_site_key') ?: env('NOCAPTCHA_SITEKEY');
        $recaptcha_version = get_static_option('recaptcha_version') ?: 'v3';
    @endphp
    @if($site_google_captcha_enable && $recaptcha_site_key)
        ...
        <div class="container mt-5">
            <div class="choose-account center-text">
                <h4 class="choose-account-title">
                    {{ get_static_option('register_page_choose_role_title') ?? __('Choose a Role') }}
                </h4>
                ...
                <div class="btn-wrapper mt-4">
                    <span class="btn-profile btn-bg-1 w-100 continue_to_info mb-6">
                        {{ get_static_option('register_page_continue_button_title') ?? __('Continue') }}
                    </span>
                </div>
            </div>
        </div>
            ...
            <div class="row gy-5 align-items-center justify-content-between">
                            ...
                            <form class="login-wrapper-form custom-form" method="post"
                                ...
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="terms_condition"
                                        name="terms_condition" style="cursor: pointer;">

                                    <label class="form-check-label" for="terms_condition" style="cursor: pointer;">
                                        {{ __('Accept all') }} <span class="fw-bold">Terms and Conditions</span> &amp; <span
                                            class="fw-bold">Privacy Policy</span>
                                    </label>
                                </div>

                                <div class="col-md-12 my-3">
                                    {{-- Honeypot field starts --}}
                                    <div style="display:none;">
                                        <input type="text" name="last_name_alt" value="" autocomplete="off" tabindex="-1">
                                    </div>
                                    {{-- Honeypot field ends --}}

                                    <input type="hidden" name="recaptchaResponse" id="recaptchaResponse">
                                    @if($site_google_captcha_enable && $recaptcha_site_key && $recaptcha_version != 'v3')
                                        <div class="g-recaptcha" data-sitekey="{{$recaptcha_site_key}}" id="recaptcha_element_register" data-callback="onRegisterCaptcha" data-expired-callback="onRegisterCaptchaExpired" data-error-callback="onRegisterCaptchaError"></div>
                                    @endif
                                    <span id="recaptcha_error" class="text-danger" style="display:none;"></span>
                                    @if ($errors->has('recaptchaResponse'))
                                        <span class="text-danger">{{ $errors->first('recaptchaResponse') }}</span>
                                    @endif
                                </div>
                                <button class="submit-btn w-100 mt-4 sign_up_now_button" type="submit">
                                    {{ __('Sign Up Now') }} <span id="user_register_load_spinner"></span></button>
                ...
                {{-- <div class="col-lg-6">--}}
                    {{-- <div class="login-right">--}}
                        {{-- <div
                            class="global-slick-init login-slider nav-style-one dot-style-one white-dot slider-inner-margin"
                            --}} {{-- data-appendArrows=".append-jobs" data-dots="true" data-infinite="true"
        ...
        (function ($) {
            ...
            $(document).ready(function () {
                ...
                function updateSocialLinks(type) {
                    $('.login-others-single-btn').each(function () {
                ...
                // Initial social link setup for default user_type (2)
                updateSocialLinks(2);
                ...
                const iti = window.intlTelInput(phoneInput, {
                    ...
                    allowDropdown: true,
                    initialCountry: "pk",
                    preferredCountries: ["pk", "sa", "us", "gb"], // Pakistan, Saudi Arabia, US, UK
                });
                ...
                $(document).on('keyup', '#phones', function () {
                    ...
                    if (phoneInput.value.trim()) {
                        if (iti.isValidNumber()) {
                            ...
                            let msg = "{{ __('Invalid phone number') }}";
                            // Basic mapping of some error codes if needed, or just generic
                ...
                // Using verify endpoint if still needed, but relying on iti.isValidNumber is safer for format.
                // If we want to check uniqueness on backend, we should use the formatted number.
                ...
                window.onRegisterCaptcha = function () {
                    $('#recaptcha_error').hide();
                    $('.sign_up_now_button').prop('disabled', false);
                };
                window.onRegisterCaptchaExpired = function () {
                    $('.sign_up_now_button').prop('disabled', true);
                    $('#recaptcha_error').text('Captcha expired. Please try again.').show();
                };
                window.onRegisterCaptchaError = function () {
                    $('.sign_up_now_button').prop('disabled', true);
                    $('#recaptcha_error').text('Captcha error. Please reload the page.').show();
                };
                ...
                $(document).on('click', '.sign_up_now_button', function (e) {
                    ...
                    let submitAjax = function (token) {
                        ...
                        $.ajax({
                            ...
                            type: 'post',
                            data: { user_type: user_type, first_name: first_name, last_name: last_name, username: username, email: email, phone: phone, password: password, confirm_password: confirm_password, terms_condition: terms_condition, recaptchaResponse: token },
                            error: function (res) {
                    ...
                    let recaptchaEnabled = {{ $site_google_captcha_enable ? 'true' : 'false' }};
                    let recaptchaVersion = '{{ $recaptcha_version }}';
                    ...
                    if (recaptchaEnabled && recaptchaSiteKey) {
                        if (recaptchaVersion === 'v3') {
                            ...
                            // v2 checkbox must be solved before submitting
                            let token = '';
                            ...
                            if (!token) {
                                $('#user_register_load_spinner').html('');
                                $('#recaptcha_error').text('Please complete the reCAPTCHA.').show();
                                $('.sign_up_now_button').prop('disabled', true);
                                return;
                            }
                            submitAjax(token);
                ...
                $(document).on('keyup', '#first_name, #last_name', function () {
                    ...
                    $(".first_name_error, .last_name_error").html("");

                    // Check for invalid characters in First Name
                    if (firstName.length > 0 && !nameRegex.test(firstName)) {
                        $('.first_name_error').html("<span style='color:red;'>Sorry! No special characters or numbers</span>");
                        return;
                    }

                    // Check for invalid characters in Last Name
                    if (lastName.length > 0 && !nameRegex.test(lastName)) {
                        $('.last_name_error').html("<span style='color:red;'>Sorry! No special characters or numbers</span>");
                        return;
                    }

                    // Check if First Name and Last Name are the same
                    if (firstName !== '' && lastName !== '' && firstName.toLowerCase() === lastName.toLowerCase()) {
                        $('.last_name_error').html("<span style='color:red;'>First name and Last name cannot be the same</span>");
                    }
                });
