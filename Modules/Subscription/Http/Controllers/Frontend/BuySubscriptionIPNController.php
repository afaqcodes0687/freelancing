...
class BuySubscriptionIPNController extends Controller
{
    ...
    public function stripe_ipn_for_order(Request $request)
    {
        ...
        $stripe->setEnv(get_static_option('stripe_test_mode') == 'on');
        ...
        if (isset($payment_data['status']) && $payment_data['status'] === 'complete') {
            ...
            // ✅ Update is_hired after successful payment
            if ($proposal_id) {
    ...
    public function authorizenet_ipn_for_subscription()
    {
        ...
        if (isset($payment_data['status']) && $payment_data['status'] === 'complete') {
            ...
            $user_type = session()->get('user_type');
            $this->update_database($order_id, $payment_data['transaction_id']);
            // record wallet history for visa/mastercard (authorize.net) payment
            try {
                $sub = UserSubscription::find($order_id);
                if ($sub && $user_id) {
                    WalletHistory::create([
                        ...
                        'amount' => $sub->price,
                        'transaction_id' => $payment_data['transaction_id'] ?? ('anet-' . $order_id),
                        'status' => 1,
                    ]);
                }
            } catch (\Exception $e) {
    ...
    private function update_database($last_subscription_id, $transaction_id)
    {
        ...
        UserSubscription::where('user_id', $subscription_details->user_id)
        ...
        try {
            ...
            \Log::error("Affiliate Subscription Commission Error: " . $e->getMessage());
        }
    }
}
