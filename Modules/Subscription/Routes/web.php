...
Route::group(['middleware' => ['globalVariable', 'maintains_mode','setlang', 'check.subscription']], function () {
    ...
    // buy subscription
    Route::controller(BuySubscriptionController::class)->group(function () {

        // Buy subscription (ONLY ONE PLACE)
        Route::post('packages/buy', 'buy_subscription')->name('subscriptions.buy');

        // Payment cancel page
        Route::get(
            'subscriptions/cancel-static',
            'subscription_payment_cancel_static'
        )->name('subscriptions.buy.payment.cancel.static');

        // PayPro return URL
        Route::get(
            'packages/paypro/return',
            'paypro_return'
        )->name('subscriptions.paypro.return');
Route::post(
            'packages/paypro/return',
            'paypro_return'
        )->name('subscriptions.paypro.return');
        // PayPro webhook (CSRF disabled)
        Route::post(
            'packages/paypro/webhook',
            'handlePayProWebhook'
        )->name('subscriptions.paypro.webhook')
