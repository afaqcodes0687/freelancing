...
class GeneralSettingsService
{
    ...
    public function social_login_settings($request)
    {
        ...
        setEnvValue([
            ...
            'FACEBOOK_CALLBACK_URL' => url('facebook/callback'),
            'GOOGLE_CLIENT_ID' => "'$request->google_client_id'",
            'GOOGLE_CLIENT_SECRET' => "'$request->google_client_secret'",
            'GOOGLE_CALLBACK_URL' => url('google/callback'),
        ]);
    ...
    public function affiliate_settings($request)
    {
        $request->validate([
            'affiliate_first_purchase_percent' => 'required|numeric|min:0|max:100',
            'affiliate_recurring_percent' => 'required|numeric|min:0|max:100',
            'affiliate_min_payout' => 'required|numeric|min:1',
        ]);

        update_static_option('affiliate_first_purchase_percent', $request->affiliate_first_purchase_percent);
        update_static_option('affiliate_recurring_percent', $request->affiliate_recurring_percent);
        update_static_option('affiliate_min_payout', $request->affiliate_min_payout);

        toastr_success(__('Affiliate Settings Updated Successfully.'));
        return back();
    }
}
