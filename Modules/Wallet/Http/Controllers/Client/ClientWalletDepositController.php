...
class ClientWalletDepositController extends Controller
{
    ...
    {
        $payfast = \App\Helper\PaymentGatewayRequestHelper::payfast();
    ...
    private function update_database($last_deposit_id, $transaction_id)
    {
        ...
        // ✅ Create affiliate commission for wallet deposit
        try {
            app(\App\Services\AffiliateCommissionService::class)->createGeneric(
                (int) $deposit_details->user_id,
                (float) $deposit_details->amount,
                "Commission from wallet deposit #{$last_deposit_id}"
            );
        } catch (\Exception $e) {
            \Log::error("Affiliate Client Wallet Deposit Commission Error: " . $e->getMessage());
        }
    }
    ...
    private function update_database_subscription($last_subscription_id, $transaction_id)
    {
        ...
        try {
            app(\App\Services\AffiliateCommissionService::class)->createGeneric(
                (int) $subscription_details->user_id,
                (float) $subscription_details->price,
                "Commission from subscription purchase #{$last_subscription_id}"
            );
        } catch (\Exception $e) {
            \Log::error("Affiliate Client Subscription Commission Error: " . $e->getMessage());
        }
    }
    ...
    private function update_database_for_order($last_order_id, $transaction_id, $user_id, $freelancer_id, $project_or_job, $proposal_id)
    {
        ...
        try {
            app(\App\Services\AffiliateCommissionService::class)->createForOrder($last_order_id);
        } catch (\Exception $e) {
            \Log::error("Affiliate Client Order Commission Error: " . $e->getMessage());
        }
    }
    ...
    private function update_promotion_database($last_package_id, $transaction_id)
    {
        ...
        // ✅ Create affiliate commission for promotion purchase
        try {
            app(\App\Services\AffiliateCommissionService::class)->createGeneric(
                (int) $promoted_package_details->user_id,
                (float) $promoted_package_details->price,
                "Commission from promotion package purchase #{$last_package_id}"
            );
        } catch (\Exception $e) {
            \Log::error("Affiliate Client Promotion Commission Error: " . $e->getMessage());
        }
    }

    public function paypro_return_for_wallet(Request $request)
    {
        $sessionCtx = session('wallet_paypro');
        $depositId = $sessionCtx['deposit_id'] ?? null;

        if (!$depositId) {
            return redirect()->route('client.wallet.history')->with(['msg' => __('Deposit not found or session expired.'), 'type' => 'error']);
        }

        $deposit = WalletHistory::find($depositId);
        if (!$deposit) {
            return redirect()->route('client.wallet.history')->with(['msg' => __('Deposit not found.'), 'type' => 'error']);
        }
        ...
        try {
            ...
            if (!$authResponse->ok()) {
                \Log::error('PayPro Authentication Failed', ['response' => $authResponse->json()]);
            ...
            if (!$checkOrderNumber) {
                \Log::error('PayPro: Order number missing from session', ['session' => $sessionCtx]);
                throw new \Exception('Order number missing for verification');
            }

            $statusResponse = \Illuminate\Support\Facades\Http::withHeaders(['token' => $token])
                ->asJson()
            ...
            \Log::info('PayPro Wallet Status Check Result', ['response' => $statusData]);
            ...
            if ($apiPaid || strtolower($request->query('status')) === 'success') {
                ...
                $this->update_database($depositId, $request->query('ordId') ?? $checkOrderNumber);
                $this->send_deposit_mail($depositId, $user_id);
                
                session()->forget('wallet_paypro');
            ...
            \Log::error('PayPro Wallet Return Error', ['msg' => $e->getMessage()]);
            return redirect()->route('client.wallet.history')
