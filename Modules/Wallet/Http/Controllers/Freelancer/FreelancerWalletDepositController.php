...
class FreelancerWalletDepositController extends Controller
{
    ...
    public function paypro_return_for_wallet(Request $request)
    {
        ...
        if (!$depositId) {
            return redirect()->route('freelancer.wallet.history')->with(['msg' => __('Deposit not found or session expired.'), 'type' => 'error']);
        }

        $deposit = WalletHistory::find($depositId);
        if (!$deposit) {
            return redirect()->route('freelancer.wallet.history')->with(['msg' => __('Deposit not found.'), 'type' => 'error']);
        }
    ...
    private function update_database($last_deposit_id, $transaction_id)
    {
        ...
        try {
            app(\App\Services\AffiliateCommissionService::class)->createGeneric(
                ...
                "Commission from wallet deposit #{$last_deposit_id} ({$deposit_details->payment_gateway})"
            );
        } catch (\Exception $e) {
            \Log::error("Affiliate Wallet Deposit Commission Error: " . $e->getMessage());
        }
    }
}
