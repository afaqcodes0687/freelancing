            ...
            <form action="{{route('admin.subcategory.all')}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="seo_content">{{ __('SEO Content (HTML supported)') }}</label>
                        <textarea name="seo_content" id="seo_content" class="form-control" rows="5" placeholder="{{ __('Enter SEO Content') }}">{{ old('seo_content', '') }}</textarea>
                    </div>
                    <x-form.text :title="__('Hero CTA Text')" :type="__('text')" :name="'hero_cta_text'" :id="'hero_cta_text'" :value="old('hero_cta_text', '')" :placeholder="__('e.g. Browse Professionals')"/>
                    <x-form.text :title="__('Hero CTA Anchor')" :type="__('text')" :name="'hero_cta_anchor'" :id="'hero_cta_anchor'" :value="old('hero_cta_anchor', '')" :placeholder="__('e.g. #professionals')"/>
