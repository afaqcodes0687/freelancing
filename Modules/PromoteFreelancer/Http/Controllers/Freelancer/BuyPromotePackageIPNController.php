...
class BuyPromotePackageIPNController extends Controller
{
    ...
    /**
     * Handle PayPro Webhook for Promotion Package
     */
    public function handleWebhook(Request $request)
    {
        $payload = $request->all();
        \Log::info('PayPro Promotion Webhook Received', ['payload' => $payload]);
        ...
        // Parse order ID from OrderNumber (PROM-123-TIMESTAMP)
        $orderId = null;
        if (str_starts_with($orderNumber, 'PROM-')) {
        ...
        $promotion = PromotionProjectList::find($orderId);
        if (!$promotion) {
            return response()->json(['status' => 'error', 'message' => 'Promotion not found'], 404);
        }

        if ($promotion->payment_status === 'complete') {
            return response()->json(['status' => 'success', 'message' => 'Promotion already complete']);
        }

        if ($status === '00') {
            // PayPro status '00' means success
            $this->update_database($promotion->id, $orderNumber);
            return response()->json(['status' => 'success', 'message' => 'Promotion processed']);
        }
    ...
    private function update_database($last_package_id, $transaction_id)
    {
        ...
        try {
            ...
            \Log::error("Affiliate Promotion Commission Error: " . $e->getMessage());
        }
    }
}
