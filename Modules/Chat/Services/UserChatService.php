...
class UserChatService
{
    ...
    {
        $query = LiveChatMessage::where("live_chat_id", $livechat_id)
                       ->where("is_seen", 0)
    ...
    public static function send(int $client_id,int $freelancer_id,?string $message,int $messageFrom,$file = null,?int $project_id = null, $type = null, $proposal_id = null, $interview_message = null, $responseType = 'html'): View|Factory|array|string|Application|null
    {
        ...
        // If message was marked delivered during storage (because user was online), broadcast status update
        if($message->is_delivered == 1){
            if($messageFrom == 1){ // Client sent to Freelancer
                event(new \Modules\Chat\Events\LivechatMessageStatusEvent(
                    $message->id,
                    'delivered',
                    'livechat-freelancer-channel.' . $instance->liveChat->client_id . '.' . $instance->liveChat->freelancer_id,
                    'livechat-freelancer-status-'. $instance->liveChat->freelancer_id
                ));
            } else { // Freelancer sent to Client
                event(new \Modules\Chat\Events\LivechatMessageStatusEvent(
                    $message->id,
                    'delivered',
                    'livechat-client-channel.' . $instance->liveChat->freelancer_id . '.' . $instance->liveChat->client_id,
                    'livechat-client-status-'. $instance->liveChat->client_id
                ));
            }
        }
    ...
    private function storeMessage(int $from_user): LiveChatMessage
    {
        ...
        $recipient_id = ($from_user == 1) ? $this->liveChat?->freelancer_id : $this->liveChat?->client_id;
        $is_delivered = Cache::has('user_is_online_' . $recipient_id) ? 1 : 0;
...
//        }
    ...
    private function prepareProductDetails($project_id, $type, $proposal_id, $interview_message): array {
        ...
        return [
            ...
            'proposal_id' => $proposal_id, // <-- Yeh line yahan ho
